...
totalDivided <- totalDivided %>%
  mutate(
    ...
    Followers = (totalFollowers/1000),
    Following = (totalFollowing/100)
  ) %>%
  select(Views, Shares, Likes, Comments, NrHashtag,Followers)

totalDivided <- rbind(rep(20000,5) , rep(0,5) , totalDivided)
...
radarchart(totalDivided,axistype=1 ,
           ...
           #custom labels
           vlcex=0.8,
           title="Engagement for political area"
)
...
condition <- c("Views","Likes" , "Shares" , "Comments", "Hashtag","Followers") 
